...
int ambptb_partition(struct parsed_partitions *state)
{
	...
	for_each_child_of_node(np,  pp) {
		...
		const __be32 *reg;
		int len, a_cells, s_cells, label_min;
		u64 addr, size;
		...
		if (of_property_read_bool(pp, "read-only")) {
			printk(KERN_DEBUG "partition[%s] is set with read-only.\n", partition_name);
			state->parts[index - 1].flags |= ADDPART_FLAG_READONLY;
		}
#endif

		info = &state->parts[index - 1].info;
		label_min = min_t(int, sizeof(info->volname) - 1, strlen(partition_name) + 1);
		strncpy(info->volname, partition_name, label_min);
		info->volname[label_min] = '\0';

		snprintf(str_tmp, sizeof(str_tmp), "(%s)", partition_name);
		strlcat(state->pp_buf, str_tmp, PAGE_SIZE);

		state->parts[index - 1].has_info = true;
