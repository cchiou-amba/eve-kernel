...
{
	uintptr_t gdma_src_addr, gdma_dst_addr, src_highaddr_pitch, dst_highaddr_pitch;
	int row_count;
	...
	if (row_count > 0) {
		gdma_src_addr = (uintptr_t)src_addr & 0xFFFFFFFF;
		gdma_dst_addr = (uintptr_t)dest_addr & 0xFFFFFFFF;
		src_highaddr_pitch = (((uintptr_t)src_addr >> 32) << 16 ) | TRANSFER_2D_WIDTH;
		dst_highaddr_pitch = (((uintptr_t)dest_addr >> 32) << 16 ) | TRANSFER_2D_WIDTH;

		writel_relaxed((u32)gdma_src_addr, ambarella_gdma->regbase + GDMA_SRC_1_BASE_OFFSET);
		writel_relaxed((u32)src_highaddr_pitch, ambarella_gdma->regbase + GDMA_SRC_1_PITCH_OFFSET);
		writel_relaxed((u32)gdma_dst_addr, ambarella_gdma->regbase + GDMA_DST_BASE_OFFSET);
		writel_relaxed((u32)dst_highaddr_pitch, ambarella_gdma->regbase + GDMA_DST_PITCH_OFFSET);
...
static inline int transfer_small_unit(u8 *dest_addr, u8 *src_addr, u32 size)
{
	...
	gdma_src_addr = (uintptr_t)src_addr & 0xFFFFFFFF;
	gdma_dst_addr = (uintptr_t)dest_addr & 0xFFFFFFFF;
	src_highaddr_pitch = (((uintptr_t)src_addr >> 32) << 16 );
	dst_highaddr_pitch = (((uintptr_t)dest_addr >> 32) << 16 );
					...
					u16 src_pitch, u16 dest_pitch, u16 width, u16 height)
{
	...
	while (height > MAX_TRANSFER_2D_HEIGHT) {
		gdma_src_addr = (uintptr_t)src_addr & 0xFFFFFFFF;
		gdma_dst_addr = (uintptr_t)dest_addr & 0xFFFFFFFF;
		src_highaddr_pitch = (((uintptr_t)src_addr >> 32) << 16 ) | src_pitch;
		dst_highaddr_pitch = (((uintptr_t)dest_addr >> 32) << 16 ) | dest_pitch;
