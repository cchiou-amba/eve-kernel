...
#define AMBARELLA_UART_TX_PIO 1
#define AMBARELLA_UART_TX_DMA 2
...
static void __serial_ambarella_stop_tx(struct uart_port *port, u32 tx_fifo_fix)
{
	...
	if (amb_port->txdma_used && (amb_port->tx_in_progress == AMBARELLA_UART_TX_DMA)) {
...
static void serial_ambarella_hw_setup(struct uart_port *port)
{
	...
	if(amb_port->rtr)
		writel_relaxed(amb_port->rtr, port->membase + UART_RTR_OFFSET);
	...
	if (amb_port->txdma_used || amb_port->rxdma_used) {
		...
		if (amb_port->rxdma_used) {
			if (amb_port->rtr && (amb_port->rtr % 8 ==0))
				writel_relaxed(UART_IE_ETOI | UART_IE_ERETOI, port->membase + UART_IE_OFFSET);
			else
		...
		struct tty_port *tty)
{
	...
	pr_debug("uart%d RXPIO %d\n", amb_port->port.line, num);
...
static void serial_ambarella_dma_rx_irq(struct ambarella_uart_port *amb_port)
{
	...
	pr_debug("uart%d RXDMA %zu\n", amb_port->port.line, pending);
...
static int ambarella_uart_fill_tx_fifo(struct ambarella_uart_port *amb_port, int max_bytes)
{
	struct circ_buf *xmit = &amb_port->port.state->xmit;
	int i, cnt = 0;

	for (i = 0; i < max_bytes; i++) {
		if (!amb_port->less_reg && tx_fifo_is_full(&amb_port->port))
			break;
		writel_relaxed(xmit->buf[xmit->tail], amb_port->port.membase + UART_TH_OFFSET);
		xmit->tail = (xmit->tail + 1) & (UART_XMIT_SIZE - 1);
		amb_port->port.icount.tx++;
		cnt++;
	}
...
static irqreturn_t serial_ambarella_irq(int irq, void *dev_id)
{
	...
	//if (amb_port->port.line == 1)
	//	pr_debug("uart%d 0x%x\n", amb_port->port.line, ii);
	switch (ii & 0x0F) {
	...
	case UART_II_THR_EMPTY:
		if (amb_port->txdma_used) {
			ier = readl_relaxed(port->membase + UART_IE_OFFSET);
			writel_relaxed(ier & ~UART_IE_ETBEI, amb_port->port.membase + UART_IE_OFFSET);
			if (amb_port->tx_bytes_pio) {
				cnt = ambarella_uart_fill_tx_fifo(amb_port, amb_port->tx_bytes_pio);
				pr_debug("uart%d int TXPIO %d\n", amb_port->port.line, cnt);
				BUG_ON(cnt != amb_port->tx_bytes_pio);
			}
			amb_port->tx_in_progress = 0;
	...
	case UART_II_CHAR_TIMEOUT_FIFO_EMPTY:
		/* for chips before cv2a, need to clera ERETOI to clear the interrupt */
		ier = readl_relaxed(port->membase + UART_IE_OFFSET);
		writel_relaxed(ier & ~UART_IE_ERETOI, amb_port->port.membase + UART_IE_OFFSET);
		writel_relaxed(ier | UART_IE_ERETOI, amb_port->port.membase + UART_IE_OFFSET);
		fallthrough;
...
static void serial_ambarella_tx_dma_complete(void *args)
{
	...
	pr_debug("uart%d TXDMA %d\n", amb_port->port.line, count);
	if (uart_circ_chars_pending(xmit) < WAKEUP_CHARS)
		uart_write_wakeup(&amb_port->port);
	serial_ambarella_start_next_tx(amb_port);
		...
		unsigned long count)
{
	...
	tx_bytes = count & ~(AMBA_UART_MIN_DMA - 1);
	...
	amb_port->tx_in_progress = AMBARELLA_UART_TX_DMA;
...
static void serial_ambarella_start_next_tx(struct ambarella_uart_port *amb_port)
{
	...
	if (count < AMBA_UART_MIN_DMA) {
		ier = readl_relaxed(port->membase + UART_IE_OFFSET);
		writel_relaxed(ier | UART_IE_ETBEI, port->membase + UART_IE_OFFSET);
		amb_port->tx_in_progress = AMBARELLA_UART_TX_PIO;
		num = ambarella_uart_fill_tx_fifo(amb_port, count);
		amb_port->tx_bytes_pio = count - num;
		pr_debug("uart%d TXPIO %d\n", amb_port->port.line, num);
	} else {
...
static void serial_ambarella_start_tx(struct uart_port *port)
{
	...
	/* if transimit buffer is not allocate, just return */
	if (!port->state->xmit.buf)
		return;
	amb_port = (struct ambarella_uart_port *)(port->private_data);
...
static void serial_ambarella_rx_dma_complete(void *args)
{
	...
	tty_flip_buffer_push(port);
	pr_debug("uart%d RXDMA %ld\n", amb_port->port.line, AMBA_UART_RX_DMA_BUFFER_SIZE);
	serial_ambarella_start_rx_dma(amb_port);
...
static void serial_ambarella_stop_rx(struct uart_port *port)
{
	...
	if (amb_port->rxdma_used) {
		...
		ier = readl_relaxed(port->membase + UART_IE_OFFSET);
		writel_relaxed(ier & ~(UART_IE_ETOI | UART_IE_ERETOI), port->membase + UART_IE_OFFSET);
			...
			bool dma_to_memory)
{
	...
	if (dma_to_memory) {
		...
		if (amb_port->rtr >= 32)
			dma_sconfig.src_maxburst = 32;
		else if (amb_port->rtr >= 16)
		...
		dma_sconfig.dst_maxburst = AMBA_UART_MIN_DMA;
...
static int serial_ambarella_startup(struct uart_port *port)
{
	...
	if (rval < 0) {
		dev_err(amb_port->port.dev, "Failed to register ISR for IRQ %d\n", port->irq);
		goto fail_irq;
	}
...
static int serial_ambarella_probe(struct platform_device *pdev)
{
	...
	/* check if using rx trigger threshold(rtr), record rtr if used */
	if (of_property_read_u32(pdev->dev.of_node, "amb,rtr", &amb_port->rtr) < 0) {
		amb_port->rtr = 0;
	}
