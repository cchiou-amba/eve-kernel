...
#define UDC_EP_WAIT_IDLE_MAX		800

#define RCT_ANA_PWR_OFFSET			0x50
#define RCT_ANA_PWR_MASK			0x3006
/* ==========================================================================*/
...
static void ambarella_udc_ep_wait_idle(struct ambarella_ep *ep)
{
	ktime_t timeout;
	struct ambarella_udc *udc = ep->udc;

	timeout = ktime_add_ms(ktime_get(), UDC_EP_WAIT_IDLE_MAX);
	/* Wait dma idle to disable udc ep */
	while(1) {
		ep->last_data_desc = ambarella_get_last_desc(ep);
		if (ambarella_check_dma_error(ep) != 1)
			break;
		else if (ktime_after(ktime_get(), timeout)) {
			pr_warn("ERR: Timeout waiting for dma last desc done\n");
			break;
		}
	}

	timeout = ktime_add_ms(ktime_get(), UDC_EP_WAIT_IDLE_MAX);
	while(readl(udc->base_reg + ep->ep_reg.ctrl_reg) & USB_EP_POLL_DEMAND) {
		if (ktime_after(ktime_get(), timeout)) {
			pr_warn("ERR: Timeout waiting for usb ep poll done\n");
			break;
		}
	}
...
static int ambarella_udc_probe(struct platform_device *pdev)
{
	...
	udc->phy_fixed = !!of_find_property(np, "amb,usb-phy-fixed", NULL);
	...
	if (!udc->phy_fixed) {
		...
		if (udc->rct_reg)
			regmap_update_bits(udc->rct_reg, RCT_ANA_PWR_OFFSET,
						RCT_ANA_PWR_MASK, RCT_ANA_PWR_MASK);
	}
