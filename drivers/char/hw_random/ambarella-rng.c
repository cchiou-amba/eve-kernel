...
#define	AMBA_RNG_CONTROL_V0		0x00
#define	AMBA_RNG_DATA_V0		0x04
#define	AMBA_RNG_CONTROL_V1		0xd0
#define	AMBA_RNG_DATA_V1		0xd4
...
static int ambarella_rng_read(struct hwrng *rng, void *buf, size_t max, bool wait)
{
	...
	/* start rng */
	val = readl(priv->base + priv->rng_control_offset);
	val |= BIT(1);
	writel(val, priv->base + priv->rng_control_offset);

	start_time = jiffies_to_msecs(jiffies);

	while ((readl(priv->base + priv->rng_control_offset) & BIT(1)) && wait) {
	...
	while (maxsize >= sizeof(u32)) {
		*data++ = readl(priv->base + priv->rng_data_offset + read);
		read += sizeof(u32);
		maxsize -= sizeof(u32);
	}
...
static const struct soc_device_attribute ambarella_rng_socinfo[] = {
        { .soc_id = "cv3ad685" },
        { .soc_id = "cv3ad655" },
        {/* sentinel */}
};
...
static int ambarella_rng_probe(struct platform_device *pdev)
{
	...
	soc = soc_device_match(ambarella_rng_socinfo);
	if (soc) {
		priv->rng_control_offset = AMBA_RNG_CONTROL_V1;
		priv->rng_data_offset = AMBA_RNG_DATA_V1;
	} else {
		priv->rng_control_offset = AMBA_RNG_CONTROL_V0;
		priv->rng_data_offset = AMBA_RNG_DATA_V0;
	}
