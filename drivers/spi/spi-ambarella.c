...
static void ambarella_spi_prepare_transfer(struct ambarella_spi *bus)
{
	...
	if (!bus->cs_active) {
		if (spi->chip_select < SPI_MAX_CS_NUM &&
			spi->master->cs_gpiods[spi->chip_select])
			gpiod_set_raw_value(spi->master->cs_gpiods[spi->chip_select], bus->cspol);
...
static void ambarella_spi_start_transfer(struct ambarella_spi *bus)
{
	...
	case SPI_WRITE_READ:
		if (!bus->dma_used) {
			xfer_len =
				min_t(int, len - widx, SPI_DATA_FIFO_SIZE_16);
	...
	case SPI_READ_ONLY:
		if (!bus->dma_used) {
			xfer_len =
				min_t(int, len - ridx, SPI_DATA_FIFO_SIZE_16);
			for (i = 0; i < xfer_len; i++)
				writel_relaxed(SPI_DUMMY_DATA,
					       bus->virt + SPI_DR_OFFSET);
		} else {
	...
	if (bus->dma_used) {
		...
		rxd = dmaengine_prep_slave_single(
			bus->rx_dma_chan, bus->rx_dma_phys, len, DMA_DEV_TO_MEM,
		...
		txd = dmaengine_prep_slave_single(
			bus->tx_dma_chan, bus->tx_dma_phys, len, DMA_MEM_TO_DEV,
			DMA_PREP_INTERRUPT | DMA_CTRL_ACK);
		BUG_ON(!txd);
...
static void ambarella_spi_next_transfer(void *args)
{
	...
	if (bus->xfer_id >= bus->n_xfer) {
		...
		if (!xfer->cs_change) {
			if (bus->cs_active) {
				if (spi->chip_select < SPI_MAX_CS_NUM && spi->master->cs_gpiods[spi->chip_select])
					gpiod_set_raw_value(spi->master->cs_gpiods[spi->chip_select], !(bus->cspol));
		...
		if (xfer->cs_change && bus->cs_active) {
			if (spi->chip_select < SPI_MAX_CS_NUM && spi->master->cs_gpiods[spi->chip_select])
				gpiod_set_raw_value(spi->master->cs_gpiods[spi->chip_select], !(bus->cspol));
...
static int ambarella_spi_one_message(struct spi_master *master,
				     struct spi_message *msg)
{
	int err = 0;
	struct spi_device *spi;
	...
	if ((spi->mode & SPI_LSB_FIRST) && bus->msb_first_only) {
		dev_err(&master->dev, "SPI[%d] only supports msb first tx-rx",
			master->bus_num);
		err = -EINVAL;
		goto ambarella_spi_transfer_exit;
	}
					      ...
					      bool dma_to_memory)
{
	...
	if (IS_ERR(dma_chan)) {
		...
		if (ret != -EPROBE_DEFER)
			dev_err(bus->dev, "Dma channel is not available: %d\n",
				ret);
		return ret;
	}

	dma_buf = dma_alloc_coherent(bus->dev, bus->dma_buf_size, &dma_phys,
				     GFP_KERNEL);
	if (!dma_buf) {
...
static int ambarella_spi_probe(struct platform_device *pdev)
{
	...
	master->num_chipselect = SPI_MAX_CS_NUM;
	...
	if (of_find_property(pdev->dev.of_node, "amb,dma-used", NULL)) {
		...
		tasklet_init(&bus->tasklet, ambarella_spi_tasklet,
			     (unsigned long)bus);
	}
...
exit_tx_dma_irq_free:
	if (bus->dma_used)
		...
		devm_free_irq(&pdev->dev, irq, bus);
exit_free_master:
...
int ambarella_spi_write(amba_spi_cfg_t *spi_cfg, amba_spi_write_t *spi_w)
{
	...
	if (!spi) {
		dev_err(&master->dev, "spi_device alloc error for spi[%d]\n", master->bus_num);
		return -ENOMEM;
	}
	...
	if (spi->chip_select < SPI_MAX_CS_NUM) {
		spi->cs_gpiod = master->cs_gpiods[spi->chip_select];
	} else {
		dev_err(&spi->dev, "Invalid chip select index %d\n",
			spi->chip_select);
		err = -EINVAL;
				  ...
				  amba_spi_write_then_read_t *spi_wtr)
{
	...
	err = spi_write_then_read(spi, spi_wtr->w_buffer, spi_wtr->w_size,
				 ...
				 amba_spi_write_and_read_t *spi_war)
{
	...
	if (spi->chip_select < SPI_MAX_CS_NUM) {
		...
		dev_warn(&spi->dev, "Using default CS behavior for index %d\n",
			 spi->chip_select);
		err = -EINVAL;
