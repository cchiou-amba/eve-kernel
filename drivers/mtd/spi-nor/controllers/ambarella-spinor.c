...
struct amb_spinor_pdata {
	u32 poc_ddr_nor_mask;
};
		...
		struct spinor_ctrl *data, struct spinor_ctrl *dummy)
{
	...
	if (addr != NULL) {
		...
		switch(addr->lane) {
		case 8:
			reg_ctrl |= SPINOR_CTRL_ADDR8LANE;
			break;
		case 4:
	...
	if (data != NULL) {
		...
		if (data->is_read && amba_spinor->octal_ddr_nor)
			writel_relaxed(SPINOR_DQS_EN, amba_spinor->regbase + SPINOR_DQS_OFFSET);
		else
			writel_relaxed(0, amba_spinor->regbase + SPINOR_DQS_OFFSET);
	}
...
static int amba_spinor_read_reg(struct spi_nor *nor, u8 opcode, u8 *buf, size_t len)
{
	...
	if (amba_spinor->octal_ddr_nor) {
		...
		rval = amba_spinor_send_cmd(amba_spinor, &cmd, NULL, &data, &dummy);
		if (rval < 0)
			return rval;
	} else {
		cmd.buf = &opcode;
...
{
	struct ambarella_spinor *amba_spinor = nor->priv;
	struct spinor_ctrl cmd, addr, data, dummy;
	u32 i, tail, bulk, dma_blk;
	u8 cmd_id, is_dma, data_lane;
	size_t offset = 0, remain;
	loff_t trans_addr;
	int ret;

	dev_dbg(amba_spinor->dev,
		"read(%#.2x): dummy_len:0x%x buf:%p from:%#.8x len:%#zx\n",
		nor->read_opcode, nor->read_dummy, buf, (u32)from, len);
	...
	if (amba_spinor->octal_ddr_nor) {
		...
		/* Need adjust for special ocatal ddr nor */
		dummy.len = 16;
		dummy.is_dtr = 0;
	} else {
		cmd_id = nor->read_opcode;
...
{
	struct ambarella_spinor *amba_spinor = nor->priv;
	struct spinor_ctrl cmd, addr, data;
	u32 i, tail = 0;
	u8 cmd_id;
	int ret;
	...
	i = to % (nor->params->page_size);
	if ((i + len) > nor->params->page_size)
		dev_err(amba_spinor->dev, "write data exceed the page bound \n");
	BUG_ON((i + len) > nor->params->page_size);
				 ...
				 struct device_node *np)
{
	...
	struct spi_nor *nor = &amba_spinor->nor;
	int ret;
...
static void amba_spinor_init(struct ambarella_spinor *amba_spinor)
{
	...
	writel_relaxed(SPINOR_INTR_ALL, amba_spinor->regbase + SPINOR_INTRMASK_OFFSET);

	writel_relaxed(SPINOR_INTR_ALL, amba_spinor->regbase + SPINOR_CLRINTR_OFFSET);
	...
	/* after reset fifo, the 0x28 will become 0x10, so read REG200
	 * times to clear the 0x28, this is an issue in hardware.
	 */
...
static const struct amb_spinor_pdata amba_spinor_rev0_pdata = {
	.poc_ddr_nor_mask = 0x00004000,
};

static const struct amb_spinor_pdata amba_spinor_rev1_pdata = {
	.poc_ddr_nor_mask = 0x00000000,
};

static const struct soc_device_attribute ambarella_spinor_socinfo[] = {
	{ .family = "Ambarella 5nm",	.data = &amba_spinor_rev0_pdata },
	{/* sentinel */}
};
...
static int ambarella_spinor_probe(struct platform_device *pdev)
{
	...
	struct amb_spinor_pdata	*soc_data;
	struct resource *res;
	...
	soc = soc_device_match(ambarella_spinor_socinfo);
	if (soc)
		soc_data = (struct amb_spinor_pdata *)soc->data;
	else
		soc_data = (struct amb_spinor_pdata *)&amba_spinor_rev1_pdata;

	amba_spinor->octal_ddr_nor = (poc & soc_data->poc_ddr_nor_mask) ? true : false;
	if (amba_spinor->octal_ddr_nor)
		dev_info(dev, "use octal ddr nor flash \n");
...
static int ambarella_spinor_remove(struct platform_device *pdev)
{
	...
	amba_spinor_release_dma(amba_spinor);
	dma_free_coherent(amba_spinor->dev, AMBARELLA_SPINOR_DMA_BUFFER_SIZE,
