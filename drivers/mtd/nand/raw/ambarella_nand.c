 ...
 *	2022/09/23 - [Li Chen] Port to 5.15
 *
...
#include "ambarella_nand_legacy.h"
...
static inline void
ambarella_fio_rct_reset(struct ambarella_nand_info *nand_info)
{
	regmap_write(nand_info->reg_rct, FIO_RESET_OFFSET,
		     FIO_RESET_FIO_RST | FIO_RESET_FLASH_RST);
	msleep(5);
...
static void nand_amb_enable_dsm(struct ambarella_nand_info *nand_info)
{
	...
	if (nand_info->ecc_bits == 6) {
		...
		nand_ext_ctr_reg =
			readl_relaxed(nand_info->regbase + FLASH_EX_CTR_OFFSET);
		nand_ext_ctr_reg |= NAND_EXT_CTR_SP_2X;
		writel_relaxed(nand_ext_ctr_reg,
			       nand_info->regbase + FLASH_EX_CTR_OFFSET);
	}

	writel_relaxed(fio_ctr_reg | FIO_CTR_RR,
		       nand_info->regbase + FIO_CTR_OFFSET);
	...
	writel_relaxed(dma_dsm_ctr,
		       nand_info->fdmaregbase + FDMA_DSM_CTR_OFFSET);
}
...
static void nand_amb_disable_bch(struct ambarella_nand_info *nand_info)
{
	...
	fio_ctr_reg |= FIO_CTR_RS;
	fio_ctr_reg &= ~(FIO_CTR_CO | FIO_CTR_ECC_6BIT | FIO_CTR_ECC_8BIT);
...
static int nand_bch_check_blank_page(struct ambarella_nand_info *nand_info)
{
	...
	for (i = 0; i < eccsteps; i++) {
		...
		zero_bits +=
			count_zero_bits(bsp, oob_subset, chip->ecc.strength);
		if (zero_bits > chip->ecc.strength)
			return -1;
...
static void amb_nand_set_timing(struct ambarella_nand_info *nand_info)
{
	...
	clk_div2 =
		POC_GCLK_CORE_DIV2_MASK ? !(poc & POC_GCLK_CORE_DIV2_MASK) : 0;
	...
	val = NAND_TIMING_LSHIFT24BIT(tcls) | NAND_TIMING_LSHIFT16BIT(tals) |
	      NAND_TIMING_LSHIFT8BIT(tcs) | NAND_TIMING_LSHIFT0BIT(tds);
	...
	val = NAND_TIMING_LSHIFT24BIT(tclh) | NAND_TIMING_LSHIFT16BIT(talh) |
	      NAND_TIMING_LSHIFT8BIT(tch) | NAND_TIMING_LSHIFT0BIT(tdh);
	...
	val = NAND_TIMING_LSHIFT24BIT(twp) | NAND_TIMING_LSHIFT16BIT(twh) |
	      NAND_TIMING_LSHIFT8BIT(twb) | NAND_TIMING_LSHIFT0BIT(trr);
	...
	val = NAND_TIMING_LSHIFT24BIT(trp) | NAND_TIMING_LSHIFT16BIT(treh) |
	      NAND_TIMING_LSHIFT8BIT(trb) | NAND_TIMING_LSHIFT0BIT(tceh);
	...
	val = NAND_TIMING_LSHIFT24BIT(trdelay) | NAND_TIMING_LSHIFT16BIT(tclr) |
	      NAND_TIMING_LSHIFT8BIT(twhr) | NAND_TIMING_LSHIFT0BIT(tir);
	...
	val = NAND_TIMING_LSHIFT16BIT(tww) | NAND_TIMING_LSHIFT8BIT(trhz) |
...
static irqreturn_t nand_fiodma_isr_handler(int irq, void *dev_id)
{
	...
	if ((val & (FIO_DMACTR_SD | FIO_DMACTR_CF | FIO_DMACTR_XD |
		    FIO_DMACTR_FL)) == FIO_DMACTR_FL) {
		fio_dma_sta =
			readl_relaxed(nand_info->regbase + FIO_DMASTA_OFFSET);
		...
		if (nand_amb_is_hw_bch(nand_info)) {
			nand_info->fio_ecc_sta = readl_relaxed(
				nand_info->regbase + FIO_ECC_RPT_STA_OFFSET);
			writel_relaxed(0x0, nand_info->regbase +
						    FIO_ECC_RPT_STA_OFFSET);
		}
...
static void nand_amb_setup_dma_devmem(struct ambarella_nand_info *nand_info)
{
	...
	ctrl_val = nand_info->len > 16 ? FDMA_NODC_MN_BURST_SIZE :
					       FDMA_NODC_SP_BURST_SIZE;
	ctrl_val |= FDMA_CHAN_CTR_WM | FDMA_CHAN_CTR_NI | FDMA_CHAN_CTR_EN |
		    nand_info->len;
	...
	writel_relaxed(nand_info->dmabase,
		       nand_info->fdmaregbase + FDMA_SRC_OFFSET);
	writel_relaxed(nand_info->buf_phys,
		       nand_info->fdmaregbase + FDMA_DST_OFFSET);

	if (nand_info->ecc_bits > 1) {
		/* Setup spare external DMA engine transfer */
		writel_relaxed(0x0,
			       nand_info->fdmaregbase + FDMA_SPR_STA_OFFSET);
		writel_relaxed(nand_info->dmabase,
	...
	if (size > 16) {
		ctrl_val = FIO_DMACTR_EN | FIO_DMACTR_FL | FIO_DMACTR_BLK_512B |
			   FIO_DMACTR_TS8B | size;
	} else {
		ctrl_val = FIO_DMACTR_EN | FIO_DMACTR_FL | FIO_DMACTR_BLK_16B |
			   FIO_DMACTR_TS8B | size;
	}
...
static void nand_amb_setup_dma_memdev(struct ambarella_nand_info *nand_info)
{
	...
	if (nand_info->len > 16)
		...
		ctrl_val = FDMA_CHAN_CTR_RM | FDMA_CHAN_CTR_NI |
			   FDMA_NODC_SP_BURST_SIZE;
	...
	ctrl_val |= FIO_DMACTR_EN | FIO_DMACTR_FL | FIO_DMACTR_TS8B |
		    FIO_DMACTR_RM | size;
	writel(ctrl_val, nand_info->regbase + FIO_DMACTR_OFFSET);
}
...
static int nand_amb_request(struct ambarella_nand_info *nand_info)
{
	...
	nand_ctr_reg = nand_info->control_reg;
	...
	case NAND_AMB_CMD_RESET:
		...
		writel_relaxed(nand_ctr_reg,
			       nand_info->regbase + FLASH_CTR_OFFSET);
		writel_relaxed(nand_cmd_reg,
			       nand_info->regbase + FLASH_CMD_OFFSET);
		break;
	...
	case NAND_AMB_CMD_COPYBACK:
		...
		writel_relaxed(nand_info->dst,
			       nand_info->regbase + FLASH_CFI_OFFSET);
		writel_relaxed(nand_ctr_reg,
			       nand_info->regbase + FLASH_CTR_OFFSET);
		writel_relaxed(nand_cmd_reg,
			       nand_info->regbase + FLASH_CMD_OFFSET);
	...
	case NAND_AMB_CMD_READ:
		...
		if (nand_amb_is_hw_bch(nand_info)) {
			...
			fio_ctr_reg = readl_relaxed(nand_info->regbase +
						    FIO_CTR_OFFSET);
			...
			case EC_MESE:
				nand_ctr_reg |=
					(NAND_CTR_EC_MAIN | NAND_CTR_EC_SPARE);
				fio_ctr_reg |= FIO_CTR_CO;
				break;
			case EC_MDSD:
			default:
	...
	case NAND_AMB_CMD_PROGRAM:
		...
		if (nand_amb_is_hw_bch(nand_info)) {
			...
			case EG_MESE:
				nand_ctr_reg |=
					(NAND_CTR_EG_MAIN | NAND_CTR_EG_SPARE);
				break;
			case EG_MDSD:
			default:
				break;
	...
	default:
		dev_warn(nand_info->dev, "%s: wrong command %d!\n", __func__,
			 cmd);
		errorCode = -EINVAL;
	...
	if (cmd == NAND_AMB_CMD_READ || cmd == NAND_AMB_CMD_PROGRAM) {
		timeout = wait_event_timeout(
			nand_info->wq, atomic_read(&nand_info->irq_flag) == 0x0,
			1 * HZ);
		if (timeout <= 0) {
			errorCode = -EBUSY;
			dev_err(nand_info->dev, "%s: cmd=0x%x timeout 0x%08x\n",
				__func__, cmd,
				atomic_read(&nand_info->irq_flag));
		} else {
			dev_dbg(nand_info->dev, "%ld jiffies left.\n", timeout);
		}

		if (nand_info->dma_status &
		    (FDMA_CHAN_STA_OE | FDMA_CHAN_STA_ME | FDMA_CHAN_STA_BE |
		     FDMA_CHAN_STA_RWE | FDMA_CHAN_STA_AE)) {
		...
		if ((nand_info->fio_dma_sta & FIO_DMASTA_RE) ||
			...
			u32 block_addr;
			block_addr = nand_info->addr / mtd->erasesize *
				     mtd->erasesize;
			dev_err(nand_info->dev,
				...
				"block addr=0x%x!\n",
				__func__, nand_info->fio_dma_sta, cmd,
				nand_info->addr_hi, nand_info->addr,
				nand_info->dst, (u32)nand_info->buf_phys,
				nand_info->len, nand_info->area, nand_info->ecc,
				block_addr);
			errorCode = -EIO;
			goto nand_amb_request_done;
		}
		...
		timeout = wait_event_timeout(
			nand_info->wq,
			(atomic_read(&nand_info->irq_flag) & 0x1) == 0x0,
			1 * HZ);
		if (timeout <= 0) {
			errorCode = -EBUSY;
			dev_err(nand_info->dev, "%s: cmd=0x%x timeout 0x%08x\n",
			...
			if (cmd == NAND_AMB_CMD_READID) {
				...
				*nand_info->dmabuf = readl_relaxed(
					nand_info->regbase + FLASH_STA_OFFSET);
			}
		}
	}
...
int nand_amb_read_data(struct ambarella_nand_info *nand_info, u32 page_addr,
		       dma_addr_t buf_dma, u8 area)
{
	struct mtd_info *mtd = nand_to_mtd(&nand_info->chip);
	int errorCode = 0;
	u32 addr_hi;
...
int nand_amb_write_data(struct ambarella_nand_info *nand_info, u32 page_addr,
			dma_addr_t buf_dma, u8 area)
{
	struct mtd_info *mtd = nand_to_mtd(&nand_info->chip);
	int errorCode = 0;
...
/* ==========================================================================*/
static uint8_t amb_nand_read_byte(struct nand_chip *chip)
{
...
static void amb_nand_read_buf(struct nand_chip *chip, uint8_t *buf, int len)
{
	struct ambarella_nand_info *nand_info = nand_get_controller_data(chip);
...
static void amb_nand_write_buf(struct nand_chip *chip, const uint8_t *buf,
			       int len)
{
	struct ambarella_nand_info *nand_info = nand_get_controller_data(chip);
...
static void amb_nand_select_chip(struct nand_chip *nand, int chip)
{
	struct ambarella_nand_info *nand_info = nand_get_controller_data(nand);

	if (chip > 0) {
		dev_err(nand_info->dev, "%s: Multi-Chip isn't supported yet.\n",
			__func__);
	}
}

static void amb_nand_cmd_ctrl(struct nand_chip *chip, int dat,
			      unsigned int ctrl)
{
}

static int amb_nand_dev_ready(struct nand_chip *chip)
{
...
static int amb_nand_waitfunc(struct nand_chip *chip)
{
	int status = 0;
	struct ambarella_nand_info *nand_info = nand_get_controller_data(chip);
...
static void amb_nand_cmdfunc(struct nand_chip *chip, unsigned command,
			     int column, int page_addr)
{
	...
	case NAND_CMD_READ0: {
		...
		nand_info->dma_bufpos = column;
		nand_amb_read_data(nand_info, page_addr, nand_info->dmaaddr,
				   area);
		if (nand_info->ecc_bits == 1)
			nand_amb_read_data(nand_info, page_addr,
	...
	case NAND_CMD_PAGEPROG: {
		...
		if (nand_info->seqin_column < mtd->writesize) {
			...
			if (nand_info->soft_ecc && nand_info->ecc_bits == 1) {
				nand_amb_write_data(
					nand_info, nand_info->seqin_page_addr,
					nand_info->dmaaddr + mtd->writesize,
					sp_area);
			}
		} else {
...
static void amb_nand_hwctl(struct nand_chip *chip, int mode)
{
}

static int amb_nand_calculate_ecc(struct nand_chip *chip, const u_char *buf,
				  u_char *code)
{
	struct ambarella_nand_info *nand_info = nand_get_controller_data(chip);
	...
	if (!nand_info->soft_ecc) {
		...
		chip->ecc.calculate(chip, buf, code);
		...
		amb_eccsize = nand_info->chip.ecc.size +
			      nand_info->soft_bch_extra_size;
		bch_encode(nand_info->bch, nand_info->bch_data, amb_eccsize,
			   code);
...
static int amb_nand_correct_data(struct nand_chip *chip, u_char *buf,
				 u_char *read_ecc, u_char *calc_ecc)
{
	...
	if (!nand_info->soft_ecc)
		...
		errorCode = chip->ecc.correct(chip, buf, read_ecc, calc_ecc);
	else {
		struct nand_chip *chip = &nand_info->chip;
		...
		count = bch_decode(nand_info->bch, NULL, amb_eccsize,
				   nand_info->read_ecc_rev,
				   nand_info->calc_ecc_rev, NULL, errloc);
		if (count > 0) {
			for (i = 0; i < count; i++) {
...
static int amb_nand_write_oob_std(struct nand_chip *chip, int page)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	struct ambarella_nand_info *nand_info = nand_get_controller_data(chip);
	int i, status;
...
static int ambarella_nand_init_soft_bch(struct ambarella_nand_info *nand_info)
{
	...
	nand_info->bch = bch_init(m, t, 0, false);
	if (!nand_info->bch)
		return -EINVAL;

	nand_info->errloc = devm_kzalloc(
		nand_info->dev, t * sizeof(*nand_info->errloc), GFP_KERNEL);
	if (!nand_info->errloc)
		return -ENOMEM;

	nand_info->bch_data =
		devm_kzalloc(nand_info->dev, amb_eccsize, GFP_KERNEL);
	 ...
	 * into spare are. */
	memset(nand_info->bch_data + chip->ecc.size, 0xff,
	       nand_info->soft_bch_extra_size);
...
static void ambarella_nand_init_hw(struct ambarella_nand_info *nand_info)
{
	...
	writel_relaxed(nand_info->control_reg,
		       nand_info->regbase + FLASH_CTR_OFFSET);

	amb_nand_set_timing(nand_info);
}

static int ambarella_nand_init_chip(struct nand_chip *chip)
{
	struct ambarella_nand_info *nand_info = nand_get_controller_data(chip);
	u32 poc;
...
static int ambarella_attach_chip(struct nand_chip *chip)
{
	...
	pr_info("writesize is %d\n", mtd->writesize);
	/* sanity check */
	BUG_ON(nand_info->ecc_bits != 1 && nand_info->ecc_bits != 6 &&
	       nand_info->ecc_bits != 8);
	...
	switch (nanddev_target_size(&nand_info->chip.base)) {
	case 8 * 1024 * 1024:
	...
	default:
		dev_err(nand_info->dev,
			...
			nanddev_target_size(&nand_info->chip.base));
		errorCode = -ENXIO;
		break;
	}
...
static int ambarella_nand_get_resource(struct ambarella_nand_info *nand_info,
				       struct platform_device *pdev)
{
	...
	nand_info->reg_rct =
		syscon_regmap_lookup_by_phandle(np, "amb,rct-regmap");
	...
	errorCode = request_irq(nand_info->cmd_irq, nand_fiocmd_isr_handler,
				IRQF_SHARED | IRQF_TRIGGER_HIGH, "fio_cmd_irq",
				nand_info);
	if (errorCode < 0) {
	...
	errorCode = request_irq(nand_info->dma_irq, nand_fiodma_isr_handler,
				IRQF_SHARED | IRQF_TRIGGER_HIGH, "fio_dma_irq",
				nand_info);
	if (errorCode < 0) {
	...
	errorCode = request_irq(nand_info->fdma_irq, ambarella_fdma_isr_handler,
				IRQF_SHARED | IRQF_TRIGGER_HIGH, "fdma_irq",
				nand_info);
	if (errorCode < 0) {
...
static int ambarella_nand_probe(struct platform_device *pdev)
{
	...
	nand_controller_init(&nand_info->controller);
	...
	ambarella_nand_init_chip(&nand_info->chip);
	...
	chip->legacy.read_byte = amb_nand_read_byte;
	chip->legacy.write_buf = amb_nand_write_buf;
	chip->legacy.read_buf = amb_nand_read_buf;
	chip->legacy.select_chip = amb_nand_select_chip;
	chip->legacy.cmd_ctrl = amb_nand_cmd_ctrl;
	chip->legacy.dev_ready = amb_nand_dev_ready;
	chip->legacy.waitfunc = amb_nand_waitfunc;
	chip->legacy.cmdfunc = amb_nand_cmdfunc;
	chip->options |= NAND_NO_SUBPAGE_WRITE;

	errorCode = nand_scan(chip, 1);
	if (errorCode < 0)
		goto ambarella_nand_probe_unregister_mtd;
