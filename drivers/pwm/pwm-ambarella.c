...
static int ambarella_pwm_apply(struct pwm_chip *chip, struct pwm_device *pwm,
			 const struct pwm_state *state)
{
	...
	if (state->polarity != pwm->state.polarity) {
		...
		err = ambarella_pwm_set_polarity(chip, pwm, state->polarity);
		if (err)
			return err;
	...
	err = ambarella_pwm_config(pwm->chip, pwm, state->duty_cycle, state->period);
	if (err)
		return err;

	if (!pwm->state.enabled)
		err = ambarella_pwm_enable(chip, pwm);

	return err;
}

static const struct pwm_ops ambarella_pwm_ops = {
	.apply          = ambarella_pwm_apply,
	.owner		= THIS_MODULE,
};
