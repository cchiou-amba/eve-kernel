...
#define AMBARELLA_TIMER_RATING		(500)
...
static void atomic_io_modify(void __iomem *reg, u32 mask, u32 set)
{
	unsigned long flags;
	u32 value;
...
u64 notrace ambarella_timer_read_counter(void)
{
	return (-(u64)readl_relaxed(sched_base_reg + TIMER_STATUS_OFFSET));
}
EXPORT_SYMBOL_GPL(ambarella_timer_read_counter);
		...
		struct ambarella_clksrc *amb_clksrc)
{
	...
	ctrl_reg = amb_clkevt ? amb_clkevt->ctrl_reg : amb_clksrc->ctrl_reg;
	ctrl_offset = amb_clkevt ? amb_clkevt->ctrl_offset : amb_clksrc->ctrl_offset;
	refclk = amb_clkevt ? amb_clkevt->refclk : amb_clksrc->refclk;
	...
	clk_rate = clk_get_rate(refclk);

	if (amb_clkevt) {
		if (amb_clkevt->clk_rate == clk_rate)
			goto resume_exit;

		amb_clkevt->clk_rate = clk_rate;
		clockevents_update_freq(&amb_clkevt->clkevt, clk_rate);
	} else {
		if (amb_clksrc->clk_rate == clk_rate)
			goto resume_exit;

		amb_clksrc->clk_rate = clk_rate;
...
static int __init ambarella_clocksource_init(struct device_node *np)
{
	...
	amb_clksrc->refclk = of_clk_get(np, 0);
	if (!amb_clksrc->refclk) {
		pr_err("%s: Failed to get refclk\n", __func__);
		return -ENXIO;
	}

	amb_clksrc->clk_rate = clk_get_rate(amb_clksrc->refclk);
	...
	clocksource_register_hz(clksrc, amb_clksrc->clk_rate);

	sched_base_reg = amb_clksrc->base_reg;
	sched_clock_register(ambarella_timer_read_counter, 32, amb_clksrc->clk_rate);

	arch_timer_read_counter = ambarella_timer_read_counter;
...
{
	struct ambarella_clkevt *amb_clkevt = to_ambarella_clkevt(clkevt);
	u32 cnt = amb_clkevt->clk_rate / HZ;
...
static int __init ambarella_clockevent_init(struct device_node *np)
{
	...
	amb_clkevt->refclk = of_clk_get(np, 0);
	if (!amb_clkevt->refclk) {
		pr_err("%s: Failed to get refclk\n", __func__);
		return -ENXIO;
	}

	amb_clkevt->clk_rate = clk_get_rate(amb_clkevt->refclk);
	...
	clkevt->features = CLOCK_EVT_FEAT_PERIODIC | CLOCK_EVT_FEAT_ONESHOT | CLOCK_EVT_FEAT_DYNIRQ;
	clkevt->rating = AMBARELLA_TIMER_RATING;
	clkevt->cpumask = cpu_possible_mask; // cpumask_of(0);
	...
	clockevents_config_and_register(&amb_clkevt->clkevt, amb_clkevt->clk_rate, 1, UINT_MAX);
...
static int ambarella_local_timer_starting_cpu(unsigned int cpu)
{
	...
	clockevents_config_and_register(clkevt, local_clkevt->clk_rate, 1, UINT_MAX);
...
static int __init ambarella_local_clockevent_init(struct device_node *np)
{
	...
	for_each_possible_cpu(cpu) {
		...
		local_clkevt->refclk = of_clk_get(np, 0);
		if (!local_clkevt->refclk) {
			pr_err("%s: Failed to get refclk\n", __func__);
			return -ENXIO;
		}

		local_clkevt->clk_rate = clk_get_rate(local_clkevt->refclk);
	...
	rval = cpuhp_setup_state(CPUHP_AP_ARMADA_TIMER_STARTING,
