...
static void *__iomem rpmsg_phys_to_virt(struct virtproc_info *vrp, u64 addr)
{
	if (addr < vrp->slave->start_pa || addr > vrp->slave->end_pa)
		return NULL;

	return vrp->slave->buf_va + addr - vrp->slave->start_pa;
}
...
#else
	addr = virtqueue_slave_pick_avaid_buffer(vrp->svq, &len);
	ret = rpmsg_phys_to_virt(vrp, addr);
#endif
...
#else
	err = virtqueue_slave_recycle_used_buffer(vrp->svq, 1, len + sizeof(*msg),
						  msg, GFP_KERNEL);
#endif
	if (err) {
		/*
...
#else
	err = virtqueue_slave_recycle_used_buffer(vrp->rvq, 1, len,
						  msg, GFP_KERNEL);
#endif
	if (err < 0) {
...
static void rpmsg_recv_done(struct virtqueue *rvq)
{
...
#else
	addr = virtqueue_slave_pick_avaid_buffer(rvq, &len);
	if (!addr) {
		dev_err(dev, "uhm, incoming signal, but no used buffer ?\n");
		return;
	}
	...
	while (addr) {
		msg = rpmsg_phys_to_virt(vrp, addr);
		err = rpmsg_recv_single(vrp, dev, msg, len);
		if (err)
			break;
...
static struct rpmsg_device *rpmsg_virtio_create_user(struct virtio_device *vdev,
						     const char *user)
{
	struct virtproc_info *vrp = vdev->priv;
	struct virtio_rpmsg_channel *vch;
	...
	rpdev_ctrl->dev.release = virtio_rpmsg_release_device;
	rpdev_ctrl->little_endian = virtio_is_little_endian(vrp->vdev);
	rpdev_ctrl->src = RPMSG_ADDR_ANY;

	strcpy(rpdev_ctrl->id.name, user);
	rpdev_ctrl->driver_override = (char *)user;

	rpmsg_register_device(rpdev_ctrl);

	dev_info(&vdev->dev, "Create rpmsg user device %s\n", user);
...
#else
	vrp->slave = (struct virtio_slave *)dev_get_drvdata(vdev->dev.parent);
	if (!vrp->slave)
		goto free_vrp;
#endif
...
#else
		dev_set_drvdata(&rpdev_ns->dev, &vrp->endpoints);
		err = rpmsg_slave_ns_register_device(rpdev_ns);
#endif
		if (err)
			/* vch will be free in virtio_rpmsg_release_device() */
			goto free_ctrldev;
...
#else
	rpmsg_virtio_create_user(vdev, vrp->slave->rpmsg_user);
#endif
...
#else
	dev_info(&vdev->dev, "rpmsg slave is online\n");
#endif
...
#else
	{ VIRTIO_ID_BLZNET, VIRTIO_DEV_ANY_ID },
#endif
	{ 0 },
};
